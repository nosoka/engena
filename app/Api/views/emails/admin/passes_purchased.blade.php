...
<body>
    ...
    <div>
        <br>Received a new purchase. Please find the details for the same below.<br>
        <table style="width:90%;border:1px solid #ccc;">
            <tr style="background-color:#ccc; font-weight: bold; text-align:left;">
                <th style="padding:5px;">Purchased by</th>
                <th>Reserve</th>
                <th>Pass</th>
                <th>Validity</th>
                @if($passes->count() > 1)
            ...
            <tr>
                <td style="padding:5px;">{{ $user->full_name }}</td>
                <td>{{ $pass->pass->reserve->ReserveName }}</td>
